...
<!--shortcut end-->
	<form id="orderForm" class="hide">
		<input type="hidden" name="paymentType" value="1"/>
			<c:set var="totalPrice"  value="${product.price}"/>
			<input type="hidden" name="orderproducts[0].productId" value="${product.id}"/>
			<input type="hidden" name="orderproducts[0].num" value="1"/>
			<input type="hidden" name="orderproducts[0].price" value="${product.price}"/>
			<input type="hidden" name="orderproducts[0].totalFee" value="${product.price}"/>
			<input type="hidden" name="orderproducts[0].title" value="${product.title}"/>
			<input type="hidden" name="orderproducts[0].imagePath" value="${product.image}"/>
		...
		<input type="hidden" name="ordershipping.receiverName" value="柳小龙"/>
	...
	<div class="s-content payment-info">
        ...
        <div class="way-list">
	        			        				         <div class="way-item">
			            尚商快递&nbsp;&nbsp;中小件商品&nbsp;&nbsp;<font color='#FF6600;'>工作日、双休日与假日均可送货</font> &nbsp;  
		...
		<div class="invoice-content">
				...
				<div class="ftx-04 invoice-prompt">
                    <dl class="clearfix">
                       <dt>温馨提示：</dt>
                       <dd>
                           <div>发票的开票金额不包括尚商卡/尚商E卡、优惠券和京豆支付部分</div>
		    ...
		    <table class="order-table">
			  <tbody>
			    <tr class="hide">
				  ...
				  <td class="p-price">
					<strong>
						￥<fmt:formatNumber groupingUsed="false" maxFractionDigits="2" minFractionDigits="2" value="${product.price/100 }"/>
					</strong>
				  </td>
					   ...
					   <div class="p-goods">
						  <div class="p-img"><a href="http://product.atguigu.com/product/${product.id}.html" target='_blank'><img width="52" height="52" src="${product.image}"></a></div>
							 <div class="p-detail">
								...
								<div class="p-more">商品编号：${product.id}<br />
																	<!-- icon图标预留-->
								<span id="promise_1057746" class="promise411"></span>
					  				  ...
					  				  <td class="p-price"><strong>￥<fmt:formatNumber groupingUsed="false" maxFractionDigits="2" minFractionDigits="2" value="${product.price / 100 }"/></strong>
					  </td>
					  				  <td class="p-promotion"> </td>
						...
						<div id="checkout-floatbar" class="checkout-buttons group">
							<div class="inner">
								...
								<div style="display:none" id="factoryShipCodShowDivBottom" class="dispatching">
									部分商品货到付款方式：先由尚商配送“提货单”并收款，然后厂商发货。
								</div>
							</div>
...
<div class="bt bt-w freight-tooltip hide" id="transport">
   ...
   <div class="w">
	...
	$('#bill-tip-btn').Jtips({//随点随帮tip弹出
		"content":'<a target="_blank" href="http://help.jd.com/help/question-61.html#kjfpf">如何区分尚商销售和第三方卖家销售的商品?</a>',
		"close":true,			
	    "position": 'bottom'
	});
	//<![CDATA[
